...
- `litellm/llms/anthropic/experimental_pass_through/adapters/transformation.py` L118-L127, L156-L210 — import tool_schema/tool_search helpers; `_model_supports_web_search_options` gates web_search_options reduction
- `litellm/llms/anthropic/experimental_pass_through/adapters/transformation.py` L740-L758, L782 — pass `defer_loading` through tool translation; drop hosted tools; sanitize tool input_schema via `drop_uncompilable_patterns`
- `litellm/llms/anthropic/experimental_pass_through/adapters/transformation.py` L1017-L1037 — forward deferred tools only when a tool_reference names them (or server-side search requested); web_search tool becomes `web_search_options` when model supports it
- `litellm/llms/anthropic/experimental_pass_through/adapters/transformation.py` L1181-L1205, L1254-L1266 — tool_reference blocks in messages expand to `<functions>` via `_tool_result_content`
- `litellm/llms/anthropic/experimental_pass_through/adapters/transformation.py` L263, L1168, L1199-L1201, L1286-L1289 — expansion is opt-in via `emulate_tool_search`, set only by the bridge; write-back callers (guardrails, shadow eval) keep upstream's `tool_reference` passthrough
- `litellm/llms/anthropic/experimental_pass_through/adapters/transformation.py` L1365-L1395 — `_count_openai_tool_calls`; one tool message per tool result
- `litellm/llms/anthropic/experimental_pass_through/adapters/transformation.py` L1519-L1532 — compaction-block insertion; truncated tool calls map to `max_tokens` finish reason
...
- `litellm/llms/anthropic/experimental_pass_through/adapters/streaming_iterator.py` L569-L591, L810-L832 — guard empty `chunk.choices` before indexing; skip translation of choice-less chunks
- `litellm/llms/anthropic/experimental_pass_through/adapters/streaming_iterator.py` L1053-L1090 — do not treat choice-less chunk as final
...
- `litellm/llms/anthropic/experimental_pass_through/messages/handler.py` L146, L639-L677 — thread `model_info` into `_should_route_to_responses_api` routing hook
...
- `litellm/llms/anthropic/experimental_pass_through/responses_adapters/transformation.py` L611-L650, L783-L858 — translate web_search tool to Responses `web_search`; Responses-API `tool_choice` conversion using translated tool list
- `litellm/llms/anthropic/experimental_pass_through/responses_adapters/streaming_iterator.py` L11-L45, L82-L87 — helpers for content-block events; web-search state on wrapper
- `litellm/llms/anthropic/experimental_pass_through/responses_adapters/streaming_iterator.py` L120-L183, L227-L329 — record search sources from citations; queue `server_tool_use` + paired `web_search_tool_result` blocks; lazy thinking-block open
- `litellm/llms/anthropic/common_utils.py` L7-L8, L38-L39 — imports for web-search result types
- `litellm/llms/anthropic/common_utils.py` L70-L124 — new: `AnthropicWebSearchResult`, `web_search_result_from_source`, `build_anthropic_web_search_tool_result_block` shared by interception callback and Responses bridge
- `litellm/integrations/websearch_interception/transformation.py` L12-L15, L440-L454 — replace inline result-block dict with shared `build_anthropic_web_search_tool_result_block`
- `litellm/completion_extras/litellm_responses_transformation/transformation.py` L37, L491-L500 — drop `tools`/`tool_choice`/`parallel_tool_calls` when hosted-tool filtering empties the tool list
- `litellm/completion_extras/litellm_responses_transformation/transformation.py` L1075-L1081 — `_convert_tools_to_responses_format` skips Anthropic hosted tools
...
- `litellm/litellm_core_utils/streaming_chunk_builder_utils.py` L346-L368 — role fallback to "assistant" when no chunk carries choices (empty-stream guard)
...
- `litellm/llms/custom_httpx/llm_http_handler.py` L2978-L2980, L5508-L5534 — thread `litellm_metadata`/`request_data`/`call_type` into responses streaming call
...
- `litellm/litellm_core_utils/llm_cost_calc/utils.py` L1198-L1200 — cache-creation cost falls back to input rate when no explicit cache-write price resolves
- `litellm/litellm_core_utils/litellm_logging.py` L3774-L3806 — unwrap Responses terminal events before the `self.stream` guard so faked streams still cost
...
- `litellm/proxy/auth/auth_exception_handler.py` L96-L124, L180-L182 — `_append_requested_model_to_budget_error` appends requested model to BudgetExceededError message

## Budget controls

- `litellm/proxy/auth/auth_checks.py` L123-L124, L354-L461 — zero-cost cache keyed by (team_id, model); `_is_model_cost_zero` team-aware via `_is_cost_explicitly_configured`
- `litellm/proxy/auth/auth_checks.py` L510-L721 — `_get_deployments_for_model` (aliases, wildcard routes), `_is_model_budget_exempt` (`model_info.skip_budget_checks` on all deployments), request-model resolution, fallback-target reachability, `should_skip_budget_checks_for_model`
- `litellm/proxy/auth/auth_checks.py` L4081-L4138 — `_get_listed_models_from_access_groups` (see Model discovery)
- `litellm/proxy/auth/user_api_key_auth.py` L62, L1557-L1563, L1978-L1984, L3284-L3290 — call `_should_skip_budget_checks` with `valid_token` in the JWT, virtual-key and custom-auth paths, replacing the inline zero-cost checks
- `litellm/proxy/auth/user_api_key_auth.py` L2616, L2734-L2752 — `_should_skip_budget_checks` passes request `fallbacks` (validated list only) into `should_skip_budget_checks_for_model`
...
- `litellm/types/router.py` L184-L187 — `ModelInfo.skip_budget_checks` field

## Vertex Model Garden coding models

- `litellm/litellm_core_utils/prompt_templates/common_utils.py` L14, L1770-L1805 — `merge_system_messages_to_front` folds all system messages into one leading message
...
- `litellm/utils.py` L2973 — best-effort model lookup, None on failure
- `litellm/utils.py` L4015-L4027 — drop `stream_options` in `pre_process_non_default_params` unless `stream is True`; keeps only what upstream's `normalize_responses_api_stream_options` forwards so the Responses bridge contract holds
- `litellm/utils.py` L8212-L8217 — `ProviderConfigManager` returns model-garden config for `openai/` Vertex prefix
...
- `litellm/proxy/utils.py` L7361-L7379, L7461-L7483 — team with access groups bounded to `_get_listed_models_from_access_groups`; no fall-through to all proxy models
- `litellm/proxy/auth/auth_checks.py` L4081-L4138 — `_get_listed_models_from_access_groups` reads `listed_model_names` from team/key access groups
...
- `litellm/proxy/proxy_server.py` L10429 — dedupe access-group names into `/v1/models` list
...
- `litellm/proxy/schema.prisma` L1392, `schema.prisma` L1392 — `listed_model_names String[]` column, identical in all three schema copies (`litellm-proxy-extras/litellm_proxy_extras/schema.prisma` included, upstream CI checks they match)
...
- `tests/test_litellm/llms/anthropic/experimental_pass_through/adapters/test_anthropic_experimental_pass_through_adapters_transformation.py` L803-L889 — content translation; L3288-L3321 web-search predicate; L4270-L4329 tool_reference expansion; L4849-L4986 output config gating
...
- `tests/test_litellm/llms/custom_httpx/test_llm_http_handler.py` L3191-L3239 — responses call threading
- `tests/test_litellm/llms/databricks/test_streaming_utils.py` L1-L72 — choice-less chunk guard
- `tests/test_litellm/llms/openai/test_is_model_gpt_5_model.py` L162-L223 — gpt-5.6 version detection in deployment names
...
- `tests/test_litellm/proxy/auth/test_auth_checks.py` L5038-L5068 — model discovery route budget bypass; L7475-L7522 — listed-models fallback targets
- `tests/test_litellm/proxy/auth/test_custom_auth_end_user_budget.py` L282-L321 — custom-auth path honors budget-exempt models
- `tests/test_litellm/proxy/auth/test_model_budget_exempt.py` L1-L644 — per-model budget exemption matrix
- `tests/test_litellm/proxy/auth/test_model_checks.py` L884-L990 — sentinel strip and dedupe in model list
- `tests/test_litellm/proxy/auth/test_unmapped_model_budget_enforcement.py` L136-L138 — unmapped model enforcement update
- `tests/test_litellm/proxy/auth/test_user_api_key_auth.py` L7, L335 — skip-budget-checks signature update
- `tests/test_litellm/proxy/common_utils/test_reset_budget_job.py` L1383-L1385 — reset job signature adjustment
- `tests/test_litellm/proxy/guardrails/guardrail_hooks/test_headroom.py` L2735-L2917 — stream_options dropped on non-streaming conversion
- `tests/test_litellm/proxy/hooks/test_proxy_track_cost_callback.py` L1636-L1698 — failure log model resolution
...
- `tests/test_litellm/completion_extras/litellm_responses_transformation/test_completion_extras_litellm_responses_transformation_transformation.py` L819-L974 — hosted tool drop; L2653-L2698 — tool_choice handling
- `tests/test_litellm/test_main.py` L1333-L1378 — model garden config resolution
- `tests/test_litellm/test_utils.py` L5824-L6117 — stream_options drop unless streaming
...
- Dashboard fixtures: `AccessGroupsPage.test.tsx`, `AccessGroupEditModal.integration.test.tsx`, `useAccessGroups.test.ts`, `AccessGroupsDetailsPage.test.tsx`, `TeamMemberTab.test.tsx` — upstream fixtures carry the fork's required `listed_model_names` and `handleMemberResetSpend`
...
## Absorbed by upstream v1.101.0 (no longer carried)
