...
- `litellm/llms/anthropic/common_utils.py` L71-L125 — new: `AnthropicWebSearchResult`, `web_search_result_from_source`, `build_anthropic_web_search_tool_result_block` shared by interception callback and Responses bridge
...
- `litellm/completion_extras/litellm_responses_transformation/transformation.py` L40, L522-L531 — drop `tools`/`tool_choice`/`parallel_tool_calls` when hosted-tool filtering empties the tool list
...
- `litellm/litellm_core_utils/llm_cost_calc/utils.py` L1215-L1217 — cache-creation cost falls back to input rate when no explicit cache-write price resolves
- `litellm/litellm_core_utils/litellm_logging.py` L3841-L3873 — unwrap Responses terminal events before the `self.stream` guard so faked streams still cost
- `litellm/proxy/hooks/proxy_track_cost_callback.py` L24, L133-L147, L235-L240 — `_resolve_failure_log_model` fills model on failed spend logs from `get_model_from_request` route resolver
...
- `litellm/proxy/auth/auth_checks.py` L126-L127, L357-L464 — zero-cost cache keyed by (team_id, model); `_is_model_cost_zero` team-aware via `_is_cost_explicitly_configured`
- `litellm/proxy/auth/auth_checks.py` L514-L725 — `_get_deployments_for_model` (aliases, wildcard routes), `_is_model_budget_exempt` (`model_info.skip_budget_checks` on all deployments), request-model resolution, fallback-target reachability, `should_skip_budget_checks_for_model`
- `litellm/proxy/auth/auth_checks.py` L4096-L4153 — `_get_listed_models_from_access_groups` (see Model discovery)
- `litellm/proxy/auth/user_api_key_auth.py` L63, L1589-L1595, L2013-L2019, L3347-L3353 — call `_should_skip_budget_checks` with `valid_token` in the JWT, virtual-key and custom-auth paths, replacing the inline zero-cost checks
- `litellm/proxy/auth/user_api_key_auth.py` L2656, L2773-L2791 — `_should_skip_budget_checks` passes request `fallbacks` (validated list only) into `should_skip_budget_checks_for_model`
...
- `litellm/types/router.py` L187-L190 — `ModelInfo.skip_budget_checks` field
...
- `litellm/utils.py` L3085 — best-effort model lookup, None on failure
- `litellm/utils.py` L4126-L4138 — drop `stream_options` in `pre_process_non_default_params` unless `stream is True`; keeps only what upstream's `normalize_responses_api_stream_options` forwards so the Responses bridge contract holds
- `litellm/utils.py` L8357-L8362 — `ProviderConfigManager` returns model-garden config for `openai/` Vertex prefix
...
- `litellm/proxy/utils.py` L7879-L7897, L7979-L8001 — team with access groups bounded to `_get_listed_models_from_access_groups`; no fall-through to all proxy models
- `litellm/proxy/auth/auth_checks.py` L4096-L4153 — `_get_listed_models_from_access_groups` reads `listed_model_names` from team/key access groups
...
- `litellm/proxy/proxy_server.py` L10721 — dedupe access-group names into `/v1/models` list
...
- `litellm/proxy/schema.prisma` L1395, `schema.prisma` L1395 — `listed_model_names String[]` column, identical in all three schema copies (`litellm-proxy-extras/litellm_proxy_extras/schema.prisma` included, upstream CI checks they match)
...
- `tests/test_litellm/llms/anthropic/test_anthropic_common_utils.py` L1698 — web-search block builder
...
- `tests/test_litellm/llms/custom_httpx/test_llm_http_handler.py` L3718-L3766 — responses call threading
...
- `tests/test_litellm/proxy/auth/test_auth_checks.py` L5117-L5147 — model discovery route budget bypass; L7475-L7522 — listed-models fallback targets
...
- `tests/test_litellm/proxy/auth/test_model_checks.py` L902-L1008 — sentinel strip and dedupe in model list
- `tests/test_litellm/proxy/auth/test_unmapped_model_budget_enforcement.py` L136-L138 — unmapped model enforcement update
- `tests/test_litellm/proxy/auth/test_user_api_key_auth.py` L367 — skip-budget-checks signature update; the fork's `timezone` import fix was absorbed by upstream v1.102.1
- `tests/test_litellm/proxy/common_utils/test_reset_budget_job.py` L1434-L1436 — reset job signature adjustment
- `tests/test_litellm/proxy/guardrails/guardrail_hooks/test_headroom.py` L2819-L3001 — stream_options dropped on non-streaming conversion
- `tests/test_litellm/proxy/hooks/test_proxy_track_cost_callback.py` L1717-L1779 — failure log model resolution
...
- `tests/test_litellm/test_main.py` L1343-L1388 — model garden config resolution
- `tests/test_litellm/test_utils.py` L6284-L6781 — stream_options drop unless streaming
