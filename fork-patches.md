...
| | |
| --- | --- |
| Upstream base | v1.99.1 |
| Fork commits | 1 (was 6) |
| Files touched | 114 |
| Lines changed | 8,954 (42% tests) |
...
**Background polls double billed.** Retrieving a stored Responses API result re-ran the cost calculator on the full usage, so a client polling a long-running background job was charged again on every poll. Retrieval is now free, which is what the provider bills
...
**Team member spend reset.** A per-team member budget could only be cleared by waiting out the cycle or raising the limit. There is now an admin endpoint and a Members-tab button that zero the current cycle while preserving lifetime spend and the log history, with the same admin permission check the rest of team management uses
...
**Template thinking off by default.** Qwen and GLM chat templates think unless told otherwise, burning tokens on requests where the client never asked for reasoning. When a request reaches a Qwen or GLM deployment without thinking or reasoning_effort enabled, the bridge switches thinking off through `chat_template_kwargs` in extra_body, with the key chosen per family: `enable_thinking: false` for Qwen and `thinking: false` for GLM. GLM 5.3 Flash ignores `enable_thinking`, and sending it also stopped the endpoint from separating reasoning, so replies came back as one text block holding the reasoning, a stray `</think>` and the answer, `reasoning_content` was empty, and the bridge could not emit an Anthropic thinking block. Measured on the dev and staging gateways against `vertex_ai/openai/glm-5-3-flash`, on `/v1/chat/completions` where this patch does not run so the injected params were the only variable: with `enable_thinking` 0/3 replies separated reasoning and 3/3 carried the inline tag, with `thinking` 3/3 separated, and completion tokens were unchanged either way, so the old key saved nothing on GLM. Clients that read the text block under a tight max_tokens, Claude Code auto mode's classifier among them, were truncated mid-reasoning by the inlined text and failed closed. The Qwen key is unchanged because it demonstrably works there, completion tokens dropping from 42 to 2 on a one-word prompt. GLM 5.2 was not reachable to validate live; `thinking` is used for every GLM deployment because it preserves reasoning separation, which is the correctness property, and any token saving is secondary
