...
def _parse_output_item(item: object) -> tuple[_ResponseOutputItem, ...]:
    ...
    if isinstance(item, ResponseOutputMessage):
        return tuple(
            _Text(
                text=(getattr(part, "refusal", "") or "") if getattr(part, "type", None) == "refusal" else getattr(part, "text", "") or "",
    ...
    if item_type == "message":
        ...
        return tuple(
            _Text(
                text=str(part.get("refusal") or "") if part.get("type") == "refusal" else str(part.get("text") or ""),
                citations=_url_citations(part.get("annotations")) if part.get("type") == "output_text" else (),
            )
            for part in texts
