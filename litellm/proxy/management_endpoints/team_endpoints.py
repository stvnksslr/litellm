...
async def team_member_reset_spend(
    ...
    if membership is None:
        raise HTTPException(
            status_code=404,
            detail={
                "error": "No team membership found for user_id={} in team_id={}".format(
                    received_user_id, data.team_id
                )
            },
        )
    ...
    await ResetBudgetJob._invalidate_spend_counter(
        f"spend:team_member:{received_user_id}:{data.team_id}"
    )
    await ResetBudgetJob._invalidate_user_api_key_cache_entry(
        f"{data.team_id}_{received_user_id}"
    )
