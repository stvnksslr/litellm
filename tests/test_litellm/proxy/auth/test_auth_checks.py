...
@pytest.mark.parametrize("route", ["/key/info", "/user/info"])
@pytest.mark.asyncio
async def test_non_inference_route_bypasses_team_budget(route):
    """Budget exhaustion gates spend-bearing (LLM) routes only. Read-only info/spend
    routes must stay reachable so an over-budget caller can still introspect.
    This intentionally widens the prior #27923 carve-out, which only spared model-discovery
    routes, to every route is_llm_api_route() classifies as non-spending. The
    side-effectful health routes are the exception and stay budget-enforced; see
    BUDGET_ENFORCED_SIDE_EFFECT_ROUTES and
