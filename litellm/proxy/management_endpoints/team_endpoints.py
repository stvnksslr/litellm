...
async def team_member_reset_spend(
    ...
    received_user_id: str | None = data.user_id
    ...
    if received_user_id is None:
        raise HTTPException(
            status_code=400,
            detail={"error": f"User id doesn't exist in team. Data={data}"},
        )
    ...
    if membership is None:
        raise HTTPException(
            status_code=404,
            detail={"error": f"No team membership found for user_id={received_user_id} in team_id={data.team_id}"},
        )
